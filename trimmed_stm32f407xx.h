 ...
 ******************************************************************************
 * @file    stm32f407xx.h
 * @author  STMicroelectronics (CMSIS Base Reference)
 * @brief   STM32F407xx Device Header - Peripheral Access Layer
 *
 * @details
 * This header defines the core register mappings, base addresses, and
 * structure definitions for every peripheral present on the STM32F407 family.
 * It serves as the low-level hardware abstraction layer at the register level,
 * linking C code directly to the MCU hardware.
 *
 * ----------------------------------------------------------------------------
 * CONTENT OVERVIEW
 * ----------------------------------------------------------------------------
 * 1. CMSIS Core Integration
 *    - Provides Cortex-M4 processor core definitions (NVIC, SCB, SysTick, etc.)
 *    - Includes standard data types and core register structures.
 *
 * 2. Memory Map and Base Addresses
 *    - Defines main memory regions: FLASH, SRAM1, SRAM2, CCM, Peripheral, FSMC.
 *    - Defines base addresses for AHB1, AHB2, AHB3, APB1, and APB2 peripheral buses.
 *
 * 3. Peripheral Register Structures
 *    - C struct representations for peripheral registers such as:
 *      GPIO_TypeDef, RCC_TypeDef, EXTI_TypeDef, SYSCFG_TypeDef,
 *      ADC_TypeDef, USART_TypeDef, SPI_TypeDef, I2C_TypeDef, TIM_TypeDef,
 *      CAN_TypeDef, DMA_TypeDef, NVIC_TypeDef, and others.
 *
 * 4. Peripheral Base Macros
 *    - Macros to access peripheral pointers, for example:
 *      #define GPIOA   ((GPIO_TypeDef *) GPIOA_BASE)
 *      #define RCC     ((RCC_TypeDef  *) RCC_BASE)
 *
 * 5. Bitfield and Register Definitions
 *    - Bit masks and position macros for control and status registers.
 *    - Enables symbolic register access instead of numeric constants.
 *      Example:
 *          RCC_CR_HSION_Pos, RCC_CR_HSION_Msk, RCC_CR_PLLON
 *
 * 6. Peripheral Enable Macros
 *    - Inline macros for enabling and disabling peripheral clocks.
 *      Example:
 *          #define RCC_AHB1ENR_GPIOAEN   (1U << 0)
 *          #define RCC_APB2ENR_USART1EN  (1U << 4)
 *
 * 7. Interrupt Vector Definitions
 *    - Enumeration of IRQ numbers and integration with CMSIS NVIC functions.
 *
 * 8. Version, Device ID, and Revision
 *    - Constants identifying the MCU device and revision.
 *
 * ----------------------------------------------------------------------------
 * PURPOSE
 * ----------------------------------------------------------------------------
 * - Provides direct register-level access for bare-metal programming.
 * - Forms the foundation for driver development (GPIO, RCC, UART, SPI, CAN, etc.)
 * - Makes hardware behavior transparent and systematic for learning.
 *
 * ----------------------------------------------------------------------------
 * COMPATIBILITY
 * ----------------------------------------------------------------------------
 * - Target MCU: STM32F407xx (Cortex-M4, up to 168 MHz)
 * - Toolchains: STM32CubeIDE, Keil uVision, IAR EWARM, GCC/VS Code
 * - Fully CMSIS-compliant, suitable for FreeRTOS or bare-metal use.
 *
 * ----------------------------------------------------------------------------
 * EDUCATIONAL NOTE
 * ----------------------------------------------------------------------------
 * Each peripheral is memory-mapped. Writing directly to its registers,
 * such as GPIOA->ODR = 0x0001, toggles physical hardware pins.
 * This file defines the register addresses and structures that make
 * such access possible in C.
 *
 * ----------------------------------------------------------------------------
 * Copyright (c) 2025
